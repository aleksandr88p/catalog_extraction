...
for index, item in enumerate(draft_file['data']['products']):
    if index == 0:
        for index2, column in enumerate(item):
            print(column)
            res_dict[f"column_{index2 + 1}"] = column

# with open('one_string.json', 'w', encoding='utf-8') as file:
...
#
#         series = item[4]['value'].get("label", None)
#
#         package_list = item[5]['value']
#         package_raw = package_list
#         package_dict = {}
#         for index, package_item in enumerate(package_list):
#             package_val = package_item.get('value', None)
#             package_help = package_item.get('help', {}).get('content', None)
#             package_dict[f"package {index + 1}"] = {'package_val': package_val, 'package_help': package_help}
#         package_dict['package_raw'] = package_raw
#
#
...
#         print(f"package dict: {package_dict}")
